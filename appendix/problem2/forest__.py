...
#             # 训练一个模型来预测货量
#             X = center_data[['日期']].apply(lambda x: x.dt.dayofyear).values.reshape(-1, 1)
#             y = center_data['货量'].values
#             model = RandomForestRegressor(n_estimators=100, random_state=42)
#             model.fit(X, y)
            
#             # 使用模型预测未来的货量
#             future_X = future_data['日期'].apply(lambda x: x.dayofyear).values.reshape(-1, 1)
#             future_y = model.predict(future_X)
#             for date, volume in zip(future_dates, future_y):
#                 future_volumes.append([center.replace('x0_', ''), date.strftime('%Y/%m/%d'), volume])
    
#     future_volumes_df = pd.DataFrame(future_volumes, columns=['分拣中心', '日期', '货量'])
    
#     # 考虑附件4的流通路径变更
#     future_volumes_df.set_index(['分拣中心', '日期'], inplace=True)
#     for index, row in data_changes.iterrows():
#         from_center = 'x0_' + row['始发分拣中心']
#         to_center = row['到达分拣中心']
#         if from_center in centers:
#             # 分配部分货量到新的目的地
#             future_volumes_df.loc[(to_center,), '货量'] += future_volumes_df.loc[(from_center,), '货量'] * 0.1
    ...
    for center in centers:
        ...
        if not center_data.empty:
            ...
            for date, volume in zip(future_dates, future_y):
                ...
                future_volumes.append([cleaned_center_name, date.strftime('%Y/%m/%d'), volume])
